...
smhi_csv <- function(stationID, direc=NA, version = "latest", parameter = "1", period = "corrected-archive") {
    ...
    data <- read.csv(text=csv,
                     ...
                     header = F, # no header
                     colClasses = c("character", # read date data as character
    ...
    if(is.na(direc)){
           ...
           write.csv(x = data, file = paste0(direc,"/",name_ID[1],".csv"))
        }
...
# # Pull a Single CSV -------------------------------------------------------
#
# directory <- "C:/Users/henrhans/Google Drive/KAU/Research/River_Wave/Hydrology_Data"
#
